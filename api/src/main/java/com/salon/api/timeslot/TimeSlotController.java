...
public class TimeSlotController {
    ...
    @Operation(summary = "타임슬롯 목록 조회",
               description = "특정 날짜·디자이너의 슬롯 목록. menuId를 함께 전달하면 시술 소요시간 기반으로 예약 가능한 시작 슬롯만 반환.")
    @GetMapping
    public ResponseEntity<List<TimeSlotResponse>> getSlots(
            ...
            @Parameter(description = "메뉴 ID (전달 시 소요시간 기반 연속 슬롯 필터 적용)", example = "1")
            @RequestParam(required = false) Long menuId) {
        return ResponseEntity.ok(timeSlotService.getSlots(storeId, date, staffId, status, menuId));
    }
}
